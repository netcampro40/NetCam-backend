...
import { deleteClientById, findClientByCnpj, findClientById, insertClient, listAllClients, updateClient, updateClientQrToken, } from "../../client/repository/client.admin.repository.js";
import { ensureClientQrCodesForKitCount, findQrCodeById, listQrCodesByClientId, listQrCodesByClientIds, updateQrCodeActive, updateQrCodeToken, } from "../../client/repository/clientQrCode.repository.js";
...
function serializeClient(row, qrCodes = []) {
    return {
        id: row.id,
        cnpj: row.cnpj,
...
function groupQrCodesByClient(rows) {
    const map = new Map();
...
async function loadClientWithQrCodes(id) {
...
async function generateMissingQrCodes(request, clientId, kitsSold, clientRow) {
    ...
    if (existing.length >= kitsSold) {
        request.log.info({ clientId, kits_sold: kitsSold, existingQrCodes: existing.length }, "generate_qr_codes_already_complete");
        return {
            status: 200,
            row: clientRow,
            qrCodes: existing,
    ...
    const isActiveQr = clientRow.commercialStatus === "INATIVO" ? false : clientRow.isActive;
    const { qrCodes, created } = await ensureClientQrCodesForKitCount(clientId, kitsSold, isActiveQr, (token, label) => {
...
export async function adminRoutes(app) {
    ...
    app.get("/lookup-cnpj/:cnpj", async (request, reply) => {
        ...
        try {
            ...
            if (!data)
                return reply.status(404).send({ error: "cnpj_not_found" });
    ...
    app.post("/clients", async (request, reply) => {
        ...
        if (parsed.data.commercialStatus === "INATIVO" &&
            parsed.data.isActive === true) {
            return reply.status(400).send({
                error: "cannot_activate_qr_inactive_client",
                message: "Cadastro inativo não pode ter o QR ativo. Desative o QR ou marque o cliente como ativo.",
            });
        }
        const isActiveQr = parsed.data.commercialStatus === "INATIVO" ? false : parsed.data.isActive;
        const kitsSold = parsed.data.kitsSold;
        request.log.info({ kitsSold, nomeFantasia: parsed.data.nomeFantasia, cnpj }, "create_client_received");
        try {
    ...
    app.patch("/clients/:id", async (request, reply) => {
        ...
        if (body.razaoSocial !== undefined)
            patch.razaoSocial = body.razaoSocial;
        if (body.nomeFantasia !== undefined)
            patch.nomeFantasia = body.nomeFantasia;
        if (body.addrCep !== undefined) {
            patch.addrCep = body.addrCep.replace(/\D/g, "").slice(0, 8);
        }
        if (body.addrStreet !== undefined)
            patch.addrStreet = body.addrStreet;
        if (body.addrNumber !== undefined)
            patch.addrNumber = body.addrNumber;
        if (body.addrComplement !== undefined)
            patch.addrComplement = body.addrComplement;
        if (body.addrNeighborhood !== undefined)
            patch.addrNeighborhood = body.addrNeighborhood;
        if (body.addrCity !== undefined)
            patch.addrCity = body.addrCity;
        if (body.addrState !== undefined)
            patch.addrState = body.addrState.toUpperCase().slice(0, 2);
        ...
        if (body.billingType !== undefined)
            patch.billingType = body.billingType;
        if (body.planValueCents !== undefined)
            patch.planValueCents = body.planValueCents;
        if (body.kitsSold !== undefined)
            patch.kitsSold = body.kitsSold;
        if (body.commercialStatus !== undefined)
            patch.commercialStatus = body.commercialStatus;
        if (body.isActive !== undefined)
            patch.isActive = body.isActive;
        const nextCommercial = body.commercialStatus !== undefined
        ...
        if (nextCommercial === "INATIVO") {
            if (body.isActive === true) {
                return reply.status(400).send({
                    error: "cannot_activate_qr_inactive_client",
                    message: "Não é possível ativar o QR enquanto o cadastro do cliente estiver inativo. Ative o cadastro comercial primeiro.",
                });
            }
    ...
    app.post("/clients/:id/qr-codes/generate", async (request, reply) => {
        ...
        if (!client)
            return reply.status(404).send({ error: "not_found" });
        const kitsSold = parsed.success && parsed.data?.kitsSold !== undefined
            ? parsed.data.kitsSold
            : client.kitsSold;
        request.log.info({ clientId: id, kits_sold: kitsSold, bodyKitsSold: parsed.success ? parsed.data?.kitsSold : undefined }, "generate_qr_codes_received");
        try {
    ...
    app.patch("/clients/:clientId/qr-codes/:qrId", async (request, reply) => {
        const { clientId, qrId } = request.params;
