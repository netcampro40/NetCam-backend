...
async function main() {
    ...
    const migrateArenasSql = readFileSync(resolve(process.cwd(), "sql/003_migrate_arenas_to_clients.sql"), "utf-8");
    const initSql = readFileSync(resolve(process.cwd(), "sql/001_init_clients.sql"), "utf-8");
    const addressSql = readFileSync(resolve(process.cwd(), "sql/004_address_split_from_legacy.sql"), "utf-8");
    const backfillClientsSchemaSql = readFileSync(resolve(process.cwd(), "sql/005_backfill_clients_schema.sql"), "utf-8");
    ...
    try {
        ...
        console.log("Database: migrate arenas → init clients → address fields → schema backfill → client QR codes → seed — OK.");
    }
    finally {
        await pool.end();
    }
